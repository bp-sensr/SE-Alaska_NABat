...
#| message: false
#| tbl-pos: H
#| tbl-cap: Occupancy trend estimates. Bolded rows show the estimates where lambada did not include 1 which suggest a changing trend
...
Overall Alaska species seem to be pretty stable, with even potential increases for bats like LANO, and increases in occupancy for most Myotis species.

The positive trend for LANO and MYCA in terms of activity trends must be taken with caution [@tbl-models]. Activity trends are very noisy and it can be hard to differentiate between a true trend and just a random increase in activity due to year effects. For this reason it is purdent to look at the trends over many years and if there is a consistent trend then we can assume it's real. In the report from 2023 @raeBlejwas2026Alaska they found that there was a a positive trend for LANO although it was barely not significant (p=0.05). The continue positive trend here indicated that this might be a true trend rather than a noisy thing. However, for MYCA the trend was not significant but it was negative, which makes me think that the MYCA estimate althoguh significant in this year, needs more years of data to be confirmed as true.

This is a reason why we are also exploring the use of occupancy analyses. Occupancy analyses require a much larger signal to be detected, since they simply look at whether the space is occupied or not and do not look at overall activity. This means that if the signal is detected we can be more confident that it is a true estimate. When we look at the results from our occupancy it seems as if though all Myotis species, except for MYCA are increasing \[@tbl-occ-trends; [Appendix A]\]. This could suggest that Myotis species are expanding in Southeast Alaska. This is an encouraging result especially considering that these are the species that are most likely to be impacted by WNS.

One importnat consideration is that there was a big pulse in activity across most sites in 2024. The sampling regime in 2024 was comparable with previous years and 2025, so this spike is most likely due to year-year variations in bat activity. Currently this difference is seen as trend in the models becuase of it's clear spike, but with more years of data we can dicern if this is a true trend or a blip created by an uncharacteristic year effect.

The reduced survey effort in 2025 was the result of staffing cuts by the US Forest Service and staffing levels are unlikely to improve in the near future. Maintaining the sampling in the sites and grid cells that have the most amount of continuous data is vital to ensuring that we can continue to detect changes in the populations, especially given the imminent threat of white nose syndrome. if need to reduce the number of grids would focus on keeping the grids that have been sampled the longest. Efforts can also be made to join efforts with other agencies to continue monitoring at current or new sites. One example that is being explored in BC and Alberta is engaging with industry partners (mines and wind development) to have coordinated monitoring efforts at their sites and getting them to contribute to the hub and overall work.

Efforts to help reduce the work it takes to deploy the grid cells are mainly focus around efficiencies in deployment. Grid leaders should be continued to be encouraged to use Survey123 as a form to simplify their deployment of sites. In 2024 and 2025 the online platform was not used consistently among all grids. In 2026 we can view efficiencies surrounding weather data used, specifically evaluating whether the continued use of temperature and humidity detectors at the site offer simialr results to using weather data from clsoest weather stations.

Species ranges continue to expand and a review of the data, specifically looking for new potentail species would be prudent. Specifically lookinga t the Big Borwn Bat. This bat has very similar calls to that of LANO, so we can explore using bioacoustics to do general checks for species for going out and catching the bats will be the only way to confirm range expansion.
