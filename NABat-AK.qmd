...
cap_sitesummary <- paste0(
  ...
  "NABat Grid cells that were monitored (with 2 or more stationary sites) in 2025 and 2024 appear in red and orange. Grids that have been monitored in previous years but not 2024 or 25 appear in grey.",
  if (is_html) paste0(
    "Zomming in shows specific sites (blue circles) within each grid cell (squares) and clicking any site opens a summary of species detections, including a ",
...
df_pts <- df %>%
  ...
  transmute(species_raw = Final.Species.ID, Site, ts) %>%
...
make_hourfacet_svg <- function(dd, ylim, width = 340L,
  ...
  if (is.null(dd) || nrow(dd) == 0) {
    return(sprintf(
      '<svg width="%d" height="150" xmlns="http://www.w3.org/2000/svg"><text x="%d" y="75" font-size="12" fill="#666" text-anchor="middle" font-family="sans-serif">No detections</text></svg>',
      width, width %/% 2L))
  }
  ...
  total_h <- mt + strip_h + panel_h + xlab_h
  ptop <- mt + strip_h; pbot <- ptop + panel_h
  
  # ---- x scale from THIS site's sampled dates (padded); ticks ARE those dates
  ud    <- sort(unique(dd$x))
  xr    <- range(ud)
  xpad  <- max(1, as.numeric(diff(xr)) * 0.08)
  xlo   <- xr[1] - xpad; xhi <- xr[2] + xpad
  xspan <- as.numeric(xhi - xlo); if (xspan <= 0) xspan <- 1
  maxlab <- max(2L, panel_w %/% 22L)                        # thin rotated labels to fit
  if (length(ud) > maxlab)
    ud <- ud[unique(round(seq(1, length(ud), length.out = maxlab)))]
  inpad <- 5
  xpix <- function(d, px0) px0 + inpad + (as.numeric(d - xlo) / xspan) * (panel_w - 2 * inpad)
  
  # ---- y scale: start of night on top (earliest hour at ptop) --------------
  y0 <- floor(min(ylim)); y1 <- ceiling(max(ylim)); if (y1 - y0 < 1) y1 <- y0 + 1
  yspan <- y1 - y0
  ypix <- function(nh) ptop + ((nh - y0) / yspan) * panel_h   # <- flipped vs. before
  
  title_txt <- sprintf(
    '<text x="%f" y="14" font-size="11" font-weight="bold" fill="#2c3e50" text-anchor="middle" font-family="sans-serif">%s</text>',
  ...
  panels <- paste(vapply(seq_len(nf), function(k) {
    ...
    strip <- paste0(
      sprintf('<rect x="%f" y="%f" width="%d" height="%d" fill="#d9d9d9"/>', px0, mt, panel_w, strip_h),
      sprintf('<text x="%f" y="%f" font-size="%d" font-weight="bold" fill="#333" text-anchor="middle" font-family="sans-serif">%s</text>',
              px0 + panel_w / 2, mt + strip_h - 4, font_size, htmltools::htmlEscape(fc)))
    pts <- if (nrow(sub) == 0) "" else paste(vapply(seq_len(nrow(sub)), function(i) {
      ...
      sprintf('<circle cx="%f" cy="%f" r="%.1f" fill="%s" fill-opacity="0.7"><title>%s  %s  %02d:%02d</title></circle>',
              cxx, cyy, point_r, point_fill, htmltools::htmlEscape(fc), format(sub$x[i], "%b %d"), hh, mm)
    }, character(1)), collapse = "")
  ...
  sprintf('<svg width="%d" height="%d" viewBox="0 0 %d %d" xmlns="http://www.w3.org/2000/svg">%s%s%s</svg>',
          total_w, total_h, total_w, total_h, title_txt, ygrid_labels, panels)
}
