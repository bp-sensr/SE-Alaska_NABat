...
# Code -> display name, in your report's species order
species_names <- c(
  LACI = "Hoary Bat",        LANO = "Silver-haired Bat",
  MYCA = "California Myotis", MYEV = "Long-eared Myotis",
  MYLU = "Little Brown Myotis", MYVO = "Long-legged Myotis",
  MYYU = "Yuma Myotis"
)
...
cov_labels <- c(
  ...
  Nightly.Max.Temp         = "Nightly max temp",
  Nightly.Min.Temp         = "Nightly min temp",
  Nightly.Max.RH           = "Nightly max RH",
  Nightly.Min.RH           = "Nightly min RH",
  Nightly.Precipitation    = "Nightly precipitation",
  Nightly.Mean.Windsp      = "Nightly mean wind speed",
  jNight                   = "Julian night"
)
...
ci_lab <- if (knitr::is_latex_output()) "Estimate $\\pm$ 95\\% CI" else "Estimate ± 95% CI"
...
``` r
Total detections ~ YearS + log(transect length) + (1|Grid Cell ID)+ species specific covariates
```
...
if (is_pdf) {
  ...
  save_kable(cov_kbl, file = "figures/actabund_covariates.png", zoom = 2)
  knitr::include_graphics("figures/actabund_covariates.png")
} else {
  cov_kbl
}
```
