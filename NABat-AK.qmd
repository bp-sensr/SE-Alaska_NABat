...
Site-level covariates, including distance to clutter, percent clutter, were recorded by grid leaders during deployment.
...
Statistical analyses followed closely what was established by WCSC and Dr. Carl Schwarz in 2021 [@raeBlejwas2024Alaska]. This includes conducting species-specific trend analyses examining activity patterns in the data collected to date using the (negative binomial GLMM) models and the covariates identified in @tbl-cov. We only included data from 2019 onwards since only a few of the grid cells had 2015 data, and it mostly didnt have data poitns between 2015-2019 (only a couple had 2017). This made the data more clean and the trend estimates more reliable. Also only inlcuded grid cell that had at least 5 years of data.

Model selection was done by looking at the running model with a combination of up to 4 distinct covariables. First we made sure to prevent colinearity in corabiables. and then ran each species mdoel through 387 combinations of the models. we then selected the lowest AIC across all the models fro each species to get the final covariables included in the models [@tbl-cov].
...
We used only manually verified data for the abundance trend models using data collected from transects. We only used transect data starting in 2019, and only grids that had at least 5 years of data. When a detection was flagged as a species couplet, it was counted toward each species listed in the couplet. Covariates were limited to weather variables and transect length, and candidate models excluded any grouping of variables that were collinear with one another. We ran 59 candidate models per species and selected the one with the lowest AIC [@tbl-cov-transects].
