...
In 2024 all 9 grid cells were monitored, and in 2025 a total of 6 grid sites were sampled in SE Alaska and 3 newly established sites were sampled around the Anchorage area (@fig-sites).
...
We estimated yearly trends in nightly acoustic activity for seven species across the stationary detector and mobile transect survey streams. Five species produced reportable stationary-detector models; two (Long-legged Myotis, Yuma Myotis) could not be reliably fit and are not reported [@tbl-models]. For transect abundance trends we only report on the results of Silver-haired bat and Little Brown bat since tbl-modelcovtbl-modelcovthese were the only two species were modelling results were reliable.

Silver-haired Bat (LANO) showed the clearest and most consistent signal, with activity increasing significantly over the monitoring period in both the stationary (β = 0.48, 95% CI \[0.31, 0.65\], p \< 0.001) and transect (β = 0.40, 95% CI \[0.20, 0.60\], p = 0.001) datasets, the only species for which a significant trend was recovered independently in both survey types.

Two additional species showed significant trends in the stationary data alone. Hoary Bat (LACI) declined (β = −0.41, 95% CI \[−0.77, −0.05\], p = 0.027), while California Myotis (MYCA) increased modestly (β = 0.14, 95% CI \[0.01, 0.27\], p = 0.036); neither had a reportable transect model.

Long-eared Myotis (MYEV; β = 0.06, 95% CI \[−0.07, 0.19\], p = 0.35) and Little Brown Myotis (MYLU; stationary β = 0.08, 95% CI \[−0.06, 0.22\], p = 0.24; transect β = 0.00, 95% CI \[−0.12, 0.12\], p = 0.94) showed no detectable trend, with confidence intervals spanning zero in every reportable model.

Long-legged Myotis (MYVO) and Yuma Myotis (MYYU) are not reported: MYVO fell below the detection threshold required for trend estimation, and the MYYU model failed to converge.
...
Talk about the differences in results between activity and occupancy trends. Is there a big difference? Do they tell us the same information?
